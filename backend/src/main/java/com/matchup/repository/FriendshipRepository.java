...
public interface FriendshipRepository extends JpaRepository<Friendship, Long> {
    ...
    @Query("SELECT f.id FROM Friendship f WHERE (f.user.id = :user1Id AND f.friend.id = :user2Id) OR (f.user.id = :user2Id AND f.friend.id = :user1Id)")
    Optional<Long> getFriendshipIdByUsers(@Param("user1Id") Long user1Id, @Param("user2Id") Long user2Id);

    @Query("SELECT CASE WHEN f.user.id = :user1Id THEN f.friend.id ELSE f.user.id END, CASE WHEN f.user.id = :user1Id " +
            "THEN f.friend.username ELSE f.user.username END FROM Friendship f WHERE (f.user.id = :user1Id OR f.friend.id = :user1Id) AND f.status = 'ACCEPTED'")
