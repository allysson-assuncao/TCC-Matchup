...
const InterestCard: React.FC<InterestCardProps> = ({interest, elementNumber}) => {
    ...
    return (
        <Grid item xs={12} sm={6} md={(elementNumber == 1) ? 12 : ((elementNumber == 2) ? 6 : 4 )}>
            <Card
                sx={{
                    width: 'auto',
            ...
            >
                ...
                <CardHeader
                    title={<>{interest.name}
                        {interest.added && (<Check sx={{width: "16px", height: "16px"}}/>)}
                    </>}
                ...
                />
                <CardActions>
                    <Button fullWidth variant={'contained'} ref={anchorEl} aria-describedby={id} type="button"
                            onClick={handleClick}>
                        Mais Informações
                    </Button>
                </CardActions>
                    ...
                    {({TransitionProps}) => (
                        <Fade {...TransitionProps} timeout={350}>
                            ...
                            >
                                ...
                                <CardContent sx={{padding: "0px", marginX: "15px", maxWidth: "400px"}}>
                                        ...
                                        {interest && interest.genres.length != 0
                                            && (<Stack direction={'row'} justifyContent={"start"} spacing={5}>
                                                {interest.dubbingLanguages.map((dubbingLanguage, index) => (
                                                    <Chip key={index} label={dubbingLanguage.id + ""}
                                                          style={{margin: 4}}
                                        ...
                                        {interest && interest.genres.length != 0
                                            && (<Stack direction={'row'} justifyContent={"start"} spacing={5}>
                                                {interest.subtitleLanguages.map((subtitleLanguage, index) => (
                                                    <Chip key={index} label={subtitleLanguage.id + ""}
                                                          style={{margin: 4}}
