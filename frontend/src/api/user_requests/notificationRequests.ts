...
export const getUnseenNotificationsCountByUserId = async (token: string): Promise<number> => {
    try {
        const response: AxiosResponse<number> = await axios
            .get(`${API_BASE_URL}/get-unseen-count`,
                {
                    headers: {
...
export const deleteNotification = async (notificationId: bigint | undefined): Promise<boolean> => {
    try {
        const response: AxiosResponse<boolean> = await
            axios.delete(`${API_BASE_URL}/delete-notification-by-id/${notificationId}`);
